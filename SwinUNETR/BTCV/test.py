...
parser.add_argument("--test_set", action="store_true", help="start distributed training")
...
def main():
    ...
    if args.test_set:
        with torch.no_grad():
            for i, batch in enumerate(val_loader):  # changed from val_loader to test_loader
                test_inputs = batch["image"].cuda()
                # print(batch["image"].meta)
                # exit()
                img_name = batch["image"].meta["filename_or_obj"][0].split("/")[-1]

                print(f"Inference on case {img_name}")

                original_affine = batch["image"].meta["affine"][0].numpy()  # using image affine if label is unavailable
                _, _, h, w, d = test_inputs.shape
                target_shape = (h, w, d)
                ...
                test_outputs = torch.softmax(test_outputs, dim=1).cpu().numpy()
                test_outputs = np.argmax(test_outputs, axis=1).astype(np.uint8)[0]

                print(test_outputs.shape, test_inputs.shape)
                # Optionally resample if test images are target_shape
                test_outputs = resample_3d(test_outputs, target_shape)

                # Save the predicted segmentation as NIfTI
                nib.save(
                    nib.Nifti1Image(test_outputs.astype(np.uint8), original_affine),
                    os.path.join(output_directory, img_name)
                )

                print(f"Saved output for {img_name}")
    else:
        with torch.no_grad():
            dice_list_case = []
            for i, batch in enumerate(val_loader):
                ...
                target_shape = (h, w, d)
                img_name = batch["image"].meta["filename_or_obj"].split("/")[-1]
