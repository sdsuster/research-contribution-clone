...
def datafold_read(json_file, fold_idx=1):
    # Load the JSON file
    ...
    if fold_key in folds:
        return folds[fold_key]['train'], folds[fold_key]['val']
    else:
...
def get_loader(args):
    ...
    train_files, validation_files = datafold_read(json_file=datalist_json, fold_idx=1+int(args.fold))
